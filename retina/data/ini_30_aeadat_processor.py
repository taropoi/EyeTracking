...
class AedatProcessorBase:
    ...
    def __read_raw_events_until(self, timestamp):
        ...
        if timestamp > self.highest_ts: # kk_added
            timestamp = self.highest_ts
            print(f"Warning: Requested timestamp {timestamp} is beyond the highest timestamp {self.highest_ts}. Clamping to highest timestamp.")
