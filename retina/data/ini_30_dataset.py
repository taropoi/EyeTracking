...
class Ini30Dataset:
    def __init__(
        ...
        # self.min_x, self.max_x = 471, 983 # kk_added
        self.y = self.y[(self.y.x_coord > self.min_x) & (self.y.x_coord < self.max_x)]
        self.y.x_coord -= self.min_x
        self.y.y_coord += 16
        # self.y.y_coord -= 16
        # self.y = self.y[(self.y.y_coord >= 0) & (self.y.y_coord < 512)] # kk_added
        print(len(self.y))
        self.avg_dt = 0
        self.items = 0
    ...
    def load_labels(self, index):
        ...
        # tab.center_y = 512 - (tab.center_y - 16) # kk_added
    ...
    def load_events(self, index):
        ...
        # evs_features = evs_features[valid] # kk_added
        print("原始 events 数:", len(events.timestamps()))
        print("裁剪后 events 数:", len(evs_timestamp))
    ...
    def load_dynamic_window(self, data, labels):
        ...
        for i in reversed(range(self.num_bins)):
            ...
            # print("xy[p == 0] shape:", xy[p == 0].shape)
